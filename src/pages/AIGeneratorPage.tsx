...
const SUGGESTIONS = [
  {
    icon: Globe,
    text: "What export markets are best for shea butter from Ghana?",
    label: "Market Research",
  },
  {
    icon: FileText,
    text: "Draft a brief export compliance checklist for agricultural products.",
    label: "Compliance",
  },
  {
    icon: Package,
    text: "What HS code should I use for dried cocoa beans?",
    label: "HS Codes",
  },
  {
    icon: Lightbulb,
    text: "Explain Incoterms CIF vs FOB for a first-time exporter.",
    label: "Trade Terms",
  },
];
...
export default function AIGeneratorPage() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Hello! I'm **Lexi**, your ExpoGen AI export assistant. I can help you with market research, HS codes, Incoterms, compliance questions, and trade documentation strategy. What export challenge can I help you navigate today?",
      timestamp: new Date(),
    },
  ]);
  ...
  const formatContent = (text: string) => {
    ...
    return text
      ...
      .replace(
        /(<li>.*<\/li>)/gs,
        '<ul class="list-disc list-inside space-y-1 my-2 text-gray-600 text-sm">$1</ul>',
      )
      .replace(/\n\n/g, '</p><p class="mb-2">')
      .replace(/\n/g, "<br/>");
  };
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-gray-50 pb-12">
      ...
      <div className="px-4 md:px-8 max-w-4xl mx-auto flex flex-col h-full">
        {/* Header */}
        <div className="mb-6">
          ...
          <p className="text-xs md:text-sm text-gray-500 mt-1">
            Ask Lexi anything about export compliance, trade documentation,
            market research, or HS codes.
          </p>
        </div>

        {/* Chat Window */}
        <div className="flex flex-col bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden min-h-[500px] md:min-h-[520px]">
          {/* Chat header bar */}
          <div className="flex items-center gap-2.5 px-4 md:px-5 py-3.5 border-b border-gray-100 bg-gradient-to-r from-teal-600 to-teal-500">
            ...
            <div>
              ...
              <p className="text-[10px] text-teal-100 mt-0.5">
                ExpoGen AI • Export Intelligence
              </p>
            </div>
                    ...
                    {msg.role === "assistant" ? (
                      <div
                        ...
                        dangerouslySetInnerHTML={{
                          __html: `<p class="mb-2">${formatContent(msg.content)}</p>`,
                        }}
                      />
                    ) : (
                      msg.content
          ...
          {messages.length === 1 && (
            <div className="px-4 md:px-5 pb-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
              {SUGGESTIONS.map((s) => {
                const Icon = s.icon;
                return (
                  <button
