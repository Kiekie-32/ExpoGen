...
import { Settings, User, Building, Mail, Globe, Bell, Shield, LogOut, ChevronRight, Check, Loader2 } from 'lucide-react';
...
const SECTION_ITEMS: { key: Section; label: string; icon: typeof Settings; description: string }[] = [
  { key: 'profile',       label: 'Profile & Business',   icon: User,   description: 'Manage your personal and business information' },
  { key: 'notifications', label: 'Notifications',         icon: Bell,   description: 'Control how and when you get notified' },
  { key: 'security',      label: 'Privacy & Security',    icon: Shield, description: 'Account security and data preferences' },
];
  ...
  const Toggle = ({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) => (
    ...
    >
      <span
        ...
        style={{ width: '18px', height: '18px', top: '2px', left: checked ? '20px' : '2px', transition: 'left 0.2s' }}
      />
    </button>
  );
  ...
  return (
    <main className="flex-1 overflow-y-auto bg-gray-50 p-6 md:p-8">
      <div className="max-w-5xl mx-auto space-y-6">
        {/* Header */}
        <div>
          <p className="text-[11px] text-gray-400 font-bold uppercase tracking-wider mb-1">Preferences</p>
          ...
          <div className="space-y-1">
            {SECTION_ITEMS.map(({ key, label, icon: Icon }) => (
              ...
              >
                <Icon size={15} />
                {label}
                {activeSection === key && <ChevronRight size={13} className="ml-auto text-teal-400" />}
              </button>
            ))}
          ...
          >
            ...
            {activeSection === 'profile' && (
              <div className="p-6 space-y-6">
                <div>
                  <h2 className="text-base font-bold text-gray-900 mb-0.5">Profile & Business</h2>
                  <p className="text-xs text-gray-400">Your personal and business information used across ExpoGen.</p>
                </div>
                ...
                <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                  <div className="w-14 h-14 rounded-full bg-teal-600 text-white text-xl font-bold flex items-center justify-center shrink-0">
                    {fullName.split(' ').map((n) => n[0]).join('').toUpperCase().substring(0, 2) || 'U'}
                  </div>
                  <div>
                    <p className="text-sm font-bold text-gray-800">{fullName || 'Your Name'}</p>
                    <p className="text-xs text-gray-400">{email}</p>
                    {businessName && <p className="text-xs text-teal-600 font-medium mt-0.5">{businessName}</p>}
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Export Region</label>
                    <div className="relative">
                      <Globe size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                      <select value={exportRegion} onChange={(e) => setExportRegion(e.target.value)}
                  ...
                  <div>
                    <label className="block text-xs font-bold text-gray-500 uppercase tracking-wide mb-2">Preferred Currency</label>
            ...
            {activeSection === 'notifications' && (
              <div className="p-6 space-y-6">
                <div>
                  <h2 className="text-base font-bold text-gray-900 mb-0.5">Notification Preferences</h2>
                  <p className="text-xs text-gray-400">Choose what activity triggers notifications in-app.</p>
                </div>

                <div className="space-y-3">
                  {[
                    { label: 'Document Generation', description: 'When certificates, invoices, or contracts are ready', value: notifDocuments, set: setNotifDocuments },
                    { label: 'Compliance Alerts', description: 'Missing certifications or regulatory changes', value: notifCompliance, set: setNotifCompliance },
                    { label: 'Market Intelligence', description: 'New trade opportunities and tariff updates', value: notifMarkets, set: setNotifMarkets },
                    { label: 'Platform Updates', description: 'New features and improvements to ExpoGen', value: notifUpdates, set: setNotifUpdates },
            ...
            {activeSection === 'security' && (
              <div className="p-6 space-y-6">
                <div>
                  <h2 className="text-base font-bold text-gray-900 mb-0.5">Privacy & Security</h2>
                  <p className="text-xs text-gray-400">Manage your data and account security settings.</p>
                </div>

                <div className="space-y-3">
                  {[
                    { label: 'Data stored securely', description: 'All your export data is encrypted at rest and in transit.', color: 'text-emerald-500', icon: Check },
                    { label: 'Session-based authentication', description: 'Your session is tied to your email and device securely.', color: 'text-teal-500', icon: Shield },
                    { label: 'No third-party data sharing', description: 'ExpoGen does not sell or share your business data with third parties.', color: 'text-blue-500', icon: Globe },
                    ...
                    <div key={label} className="flex items-start gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
                      <div className={`w-7 h-7 rounded-lg bg-white border border-gray-100 flex items-center justify-center shrink-0 ${color}`}>
                        <Icon size={14} />
                      </div>
                      <div>
                        <p className="text-sm font-semibold text-gray-800">{label}</p>
