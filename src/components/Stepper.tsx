            ...
            <div key={step.id} className="relative z-10 flex flex-col items-center group">
              ...
              >
                {isCompleted ? (
                  ...
                  <span className={`text-xs font-bold ${isActive ? "text-teal-600" : "text-gray-400"}`}>
                    {step.id}
                  </span>
                )}
              </div>
              <span className={`absolute top-10 text-[10px] font-bold uppercase tracking-wider whitespace-nowrap transition-colors ${
                isActive ? "text-teal-600" : isCompleted ? "text-gray-500" : "text-gray-400"
              }`}>
                {step.label}
              </span>
            </div>
